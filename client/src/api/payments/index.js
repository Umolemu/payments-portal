...
// Import token utilities from users module
import { getToken } from "../users/index.js";
...
export const getPayments = async () => {
  ...
  if (!response.ok) {
    const errorData = await response
      .json()
      .catch(() => ({ error: "Failed to fetch payments" }));
...
export const getPaymentById = async (id) => {
  ...
  if (!response.ok) {
    const error = await response
      .json()
      .catch(() => ({ error: "Failed to fetch payment" }));
...
export const sendPaymentSwift = async (id) => {
  ...
  if (!response.ok) {
    const error = await response
      .json()
      .catch(() => ({ error: "Failed to send payment" }));
...
export const getMe = async () => {
  ...
  if (!response.ok) {
    const error = await response
      .json()
      .catch(() => ({ error: "Failed to fetch user info" }));
